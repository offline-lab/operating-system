...
@test "fs::is_dir: returns 2 with no arguments" {
    run fs::is_dir
    [ "${status}" -eq 2 ]
}

@test "fs::is_file: returns 2 with no arguments" {
    run fs::is_file
    [ "${status}" -eq 2 ]
}

@test "fs::is_blockdev: returns 2 with no arguments" {
    run fs::is_blockdev
    [ "${status}" -eq 2 ]
}

@test "fs::is_socket: returns 2 with no arguments" {
    run fs::is_socket
    [ "${status}" -eq 2 ]
}

@test "fs::is_chardev: returns 2 with no arguments" {
    run fs::is_chardev
    [ "${status}" -eq 2 ]
}

@test "fs::is_pipe: returns 2 with no arguments" {
    run fs::is_pipe
    [ "${status}" -eq 2 ]
}

@test "fs::is_link: returns 2 with no arguments" {
    run fs::is_link
    [ "${status}" -eq 2 ]
}

@test "fs::is_executable: returns 2 with no arguments" {
    run fs::is_executable
    [ "${status}" -eq 2 ]
}

@test "fs::exists: returns 2 with no arguments" {
    run fs::exists
    [ "${status}" -eq 2 ]
}

@test "fs::is_port: returns 2 with no arguments" {
    run fs::is_port
    [ "${status}" -eq 2 ]
}
